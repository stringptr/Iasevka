...
function patchFont(patcherPath, fontPath, options) {
	...
	try {
		...
		const resultFile = patchedFiles.length > 0 ? patchedFiles[0] : files.find(f => f.endsWith(".ttf"));
		if (!resultFile) {
			throw new Error(`No patched font file found in ${tmpDir}`);
		}

		FS.copyFileSync(Path.join(tmpDir, resultFile), fontPath);
